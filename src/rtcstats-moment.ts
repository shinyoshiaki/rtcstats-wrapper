...
function getVideoSenderStats(last: any, prev: any) {
    const stats = {};
    if (last.has(RTCStatsReferences.RTCRemoteInboundRtpVideoStreams.key)) {
        ...
        const RTCRemoteInboundRtpVideoStreamStats = last.get(RTCStatsReferences.RTCRemoteInboundRtpVideoStreams.key)[0];
        (stats as any).jitter = RTCRemoteInboundRtpVideoStreamStats.jitter;
        (stats as any).rtt = RTCRemoteInboundRtpVideoStreamStats.roundTripTime;
    }
    if (last.has(RTCStatsReferences.RTCOutboundRtpVideoStreams.key) &&
        prev.has(RTCStatsReferences.RTCOutboundRtpVideoStreams.key)) {
        // While we only support single-track stream, this method only care about 1 transceiver.
        const RTCOutboundRtpVideoStreamStats = last.get(RTCStatsReferences.RTCOutboundRtpVideoStreams.key)[0];
        const previous = {
            RTCOutboundRtpVideoStreamStats: prev.get(RTCStatsReferences.RTCOutboundRtpVideoStreams.key)[0]
        };
        ...
        if (RTCOutboundRtpVideoStreamStats.totalEncodeTime !== null &&
            ...
            const encodeTimeDelta = RTCOutboundRtpVideoStreamStats.totalEncodeTime -
                previous.RTCOutboundRtpVideoStreamStats.totalEncodeTime;
            const framesEncodedDelta = RTCOutboundRtpVideoStreamStats.framesEncoded -
                previous.RTCOutboundRtpVideoStreamStats.framesEncoded;
            (stats as any).averageEncodeTime = encodeTimeDelta / framesEncodedDelta;
        }
        // calculate QP value
        if (RTCOutboundRtpVideoStreamStats.qpSum !== null &&
            RTCOutboundRtpVideoStreamStats.framesEncoded !== null) {
            const qpSumDelta = RTCOutboundRtpVideoStreamStats.qpSum -
                previous.RTCOutboundRtpVideoStreamStats.qpSum;
            const framesEncodedDelta = RTCOutboundRtpVideoStreamStats.framesEncoded -
                previous.RTCOutboundRtpVideoStreamStats.framesEncoded;
            (stats as any).qpValue = qpSumDelta / framesEncodedDelta;
        }
        // calculate bitrate with previous value
        if (RTCOutboundRtpVideoStreamStats.bytesSent !== null) {
            const bytesSentDelta = RTCOutboundRtpVideoStreamStats.bytesSent -
                previous.RTCOutboundRtpVideoStreamStats.bytesSent;
            const timeDelta = RTCOutboundRtpVideoStreamStats.timestamp -
                previous.RTCOutboundRtpVideoStreamStats.timestamp;
            // convert bytes/ms to bit/sec
            const bytesPerMs = bytesSentDelta / timeDelta;
            (stats as any).bitrate = bytesPerMs * 8 * 1000;
        }
    }
    return stats;
}
function getAudioSenderStats(last: any, prev: any) {
    const stats = {};
    if (last.has(RTCStatsReferences.RTCAudioSenders.key)) {
        ...
        (stats as any).audioLevel = RTCAudioSenderStats.audioLevel;
    }
    if (last.has(RTCStatsReferences.RTCRemoteInboundRtpAudioStreams.key)) {
        // While we only support single-track stream, this method only care about 1 transceiver.
        const RTCRemoteInboundRtpAudioStreamStats = last.get(RTCStatsReferences.RTCRemoteInboundRtpAudioStreams.key)[0];
        (stats as any).jitter = RTCRemoteInboundRtpAudioStreamStats.jitter;
        (stats as any).rtt = RTCRemoteInboundRtpAudioStreamStats.roundTripTime;
    }
    if (last.has(RTCStatsReferences.RTCOutboundRtpAudioStreams.key) &&
        prev.has(RTCStatsReferences.RTCOutboundRtpAudioStreams.key)) {
        // While we only support single-track stream, this method only care about 1 transceiver.
        const RTCOutboundRtpAudioStreamStats = last.get(RTCStatsReferences.RTCOutboundRtpAudioStreams.key)[0];
        const previous = {
            RTCOutboundRtpAudioStreamStats: prev.get(RTCStatsReferences.RTCOutboundRtpAudioStreams.key)[0]
        };
        // calculate bitrate with previous value
        if (RTCOutboundRtpAudioStreamStats.bytesSent !== null) {
            const bytesSentDelta = RTCOutboundRtpAudioStreamStats.bytesSent -
                previous.RTCOutboundRtpAudioStreamStats.bytesSent;
            const timeDelta = RTCOutboundRtpAudioStreamStats.timestamp -
...
function getVideoReceiverStats(last: any, prev: any) {
    const stats = {};
    if (last.has(RTCStatsReferences.RTCVideoReceivers.key) &&
        prev.has(RTCStatsReferences.RTCVideoReceivers.key)) {
        // While we only support single-track stream, this method only care about 1 transceiver.
        const RTCVideoReceiverStats = last.get(RTCStatsReferences.RTCVideoReceivers.key)[0];
        if (prev.has(RTCStatsReferences.RTCVideoReceivers.key)) {
            ...
            if (RTCVideoReceiverStats.jitterBufferDelay !== null &&
                ...
                const jitterBufferDelayDelta = RTCVideoReceiverStats.jitterBufferDelay -
                    previous.RTCVideoReceiverStats.jitterBufferDelay;
                const jBDEmittedDelta = RTCVideoReceiverStats.jitterBufferEmittedCount -
                    previous.RTCVideoReceiverStats.jitterBufferEmittedCount;
                (stats as any).jitterBufferDelay = jitterBufferDelayDelta / jBDEmittedDelta;
            }
        }
    }
    if (last.has(RTCStatsReferences.RTCInboundRtpVideoStreams.key)) {
        // While we only support single-track stream, this method only care about 1 transceiver.
        const RTCInboundRtpVideoStreamStats = last.get(RTCStatsReferences.RTCInboundRtpVideoStreams.key)[0];
        ...
        if (prev.has(RTCStatsReferences.RTCInboundRtpVideoStreams.key)) {
            const previous = {
                RTCInboundRtpVideoStreamStats: prev.get(RTCStatsReferences.RTCInboundRtpVideoStreams.key)[0]
            };
            // calculate QP value
            if (RTCInboundRtpVideoStreamStats.qpSum !== null &&
                RTCInboundRtpVideoStreamStats.framesDecoded !== null) {
                const qpSumDelta = RTCInboundRtpVideoStreamStats.qpSum -
                    previous.RTCInboundRtpVideoStreamStats.qpSum;
                const framesDecodedDelta = RTCInboundRtpVideoStreamStats.framesDecoded -
                    previous.RTCInboundRtpVideoStreamStats.framesDecoded;
                (stats as any).qpValue = qpSumDelta / framesDecodedDelta;
            }
            // calculate bitrate with previous value
            if (RTCInboundRtpVideoStreamStats.bytesReceived !== null) {
                const bytesReceivedDelta = RTCInboundRtpVideoStreamStats.bytesReceived -
                    previous.RTCInboundRtpVideoStreamStats.bytesReceived;
                const timeDelta = RTCInboundRtpVideoStreamStats.timestamp -
                    previous.RTCInboundRtpVideoStreamStats.timestamp;
                // convert bytes/ms to bit/sec
                const bytestPerMs = bytesReceivedDelta / timeDelta;
                (stats as any).bitrate = bytestPerMs * 8 * 1000;
            }
        }
    }
    return stats;
}
function getAudioReceiverStats(last: any, prev: any) {
    const stats = {};
    if (last.has(RTCStatsReferences.RTCAudioReceivers.key)) {
        // While we only support single-track stream, this method only care about 1 transceiver.
        const RTCAudioReceiverStats = last.get(RTCStatsReferences.RTCAudioReceivers.key)[0];
        (stats as any).audioLevel = RTCAudioReceiverStats.audioLevel;
        if (prev.has(RTCStatsReferences.RTCAudioReceivers.key)) {
            // While we only support single-track stream, this method only care about 1 transceiver.
            const RTCAudioReceiverStats = last.get(RTCStatsReferences.RTCAudioReceivers.key)[0];
            if (prev.has(RTCStatsReferences.RTCAudioReceivers.key)) {
                const previous = {
                    RTCAudioReceiverStats: prev.get(RTCStatsReferences.RTCAudioReceivers.key)[0]
                };
                if (RTCAudioReceiverStats.jitterBufferDelay !== null &&
                    RTCAudioReceiverStats.jitterBufferEmittedCount !== null) {
                    const jitterBufferDelayDelta = RTCAudioReceiverStats.jitterBufferDelay -
                        previous.RTCAudioReceiverStats.jitterBufferDelay;
                    const jBDEmittedDelta = RTCAudioReceiverStats.jitterBufferEmittedCount -
    ...
    if (last.has(RTCStatsReferences.RTCInboundRtpAudioStreams.key)) {
        // While we only support single-track stream, this method only care about 1 transceiver.
        const RTCInboundRtpAudioStreamStats = last.get(RTCStatsReferences.RTCInboundRtpAudioStreams.key)[0];
        // calculate fractionLost
        ...
        if (prev.has(RTCStatsReferences.RTCInboundRtpAudioStreams.key)) {
            const previous = {
                RTCInboundRtpAudioStreamStats: prev.get(RTCStatsReferences.RTCInboundRtpAudioStreams.key)[0]
            };
            // calculate bitrate with previous value
            if (RTCInboundRtpAudioStreamStats.bytesReceived !== null) {
                const bytesReceivedDelta = RTCInboundRtpAudioStreamStats.bytesReceived -
                    previous.RTCInboundRtpAudioStreamStats.bytesReceived;
                const timeDelta = RTCInboundRtpAudioStreamStats.timestamp -
...
function getCandidatePairStats(last: any, prev: any) {
    const stats = {};
    if (last.has(RTCStatsReferences.RTCIceCandidatePairs.key) &&
        last
            .get(RTCStatsReferences.RTCIceCandidatePairs.key)
            .some((stat: any) => stat.nominated)) {
        const RTCIceCandidatePairStats = last
            .get(RTCStatsReferences.RTCIceCandidatePairs.key)
            .find((stat: any) => stat.nominated);
        // assign rtt directly
        (stats as any).rtt = RTCIceCandidatePairStats.currentRoundTripTime;
        // check if previous stats also has nominated candidate-pair
        if (prev.has(RTCStatsReferences.RTCIceCandidatePairs.key) &&
            prev
                .get(RTCStatsReferences.RTCIceCandidatePairs.key)
                .some((stat: any) => stat.nominated)) {
            const previous = {
